...
Pod Snapshots require the rapid release channel. The `--enable-pod-snapshots` flag turns on the feature and installs the `PodSnapshot` CRD.
...
Once the cluster is ready, get credentials and check the CRD is installed:
...
kubectl exec $POD -- wget -qO- http://localhost:8080/ready
```
